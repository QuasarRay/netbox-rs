...
fn generate_fidelity_files(
    ...
    if decoded != source.as_bytes() {
        return Err(
            "internal fidelity failure: base64 round trip changed the OpenAPI source".into(),
        );
    }
...
fn generate_document_proto(source: &str, document: &Value, package: &str, encoded: &str) -> String {
    let mut out = String::new();
    write_header(&mut out, document, package);
...
fn write_header(out: &mut String, document: &Value, package: &str) {
    ...
    out.push_str(
        "// This file is part of the byte-for-byte lossless OpenAPI preservation layer.\n\n",
    );
...
fn verify_generated_document(path: &Path, expected: &[u8]) -> Result<(), Box<dyn Error>> {
    ...
    if decoded != expected {
        return Err(
            "generated protobuf does not reconstruct the OpenAPI input byte-for-byte".into(),
        );
    }
